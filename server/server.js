...
The following is a verbatim excerpt from a sample therapy session. It is included as a behavioral reference, not as content to replicate. Model your responses on the therapist's stance and interactional moves, not on the topic. Do not imitate the client's content.

Therapist:
I want to ask you a different kind of question. Suppose that tonight, while you are asleep, a miracle happens and the problem that brought you here is solved—but because you were asleep, you don't know that it has happened. What will be the first small sign when you wake up that would tell you things are better?

Client:
I think I'd get up more easily. I wouldn't want to hide under the blankets.

Therapist:
And if you were getting up more easily, what would you do next that would be different?

Client:
I'd probably talk to my kids more calmly in the morning.
...
// GET /api/personalities - Returns available personality options
app.get('/api/personalities', (req, res) => {
  try {
    const personalities = Object.values(PERSONALITIES).map(({ name, description }) => ({
      name,
      description,
    }));
...
// POST /api/chat - Handles chat messages with personality-specific responses
  ...
  try {
    ...
    if (!sessionId || !message || !personality) {
      return res.status(400).json({
        error: 'Missing required fields: sessionId, message, and personality are required',
      });
    }

    if (!PERSONALITIES[personality]) {
      return res.status(400).json({
        error: `Invalid personality type. Available options: ${Object.keys(PERSONALITIES).join(', ')}`,
      });
    }

    // Safety check — intercept before calling Claude to guarantee verbatim response
    ...
    // Get or initialize conversation session
    ...
    // Add user message to conversation history for this session
    ...
    // Build Claude-format messages array (exclude timestamps, use only role + content)
    ...
    try {
      ...
      console.log(`Claude response snippet: ${aiResponse.slice(0, 1200)}${aiResponse.length > 1200 ? '... [truncated in logs]' : ''}`);
    } catch (logErr) {
      console.warn('Failed to log Claude response details:', logErr);
    }

    // Add AI response to conversation history for this session
    conversationHistory[sessionId].messages.push({
      role: 'assistant',
      content: aiResponse,
      timestamp: new Date().toISOString(),
    ...
    if (sessionId && conversationHistory[sessionId] && conversationHistory[sessionId].messages && conversationHistory[sessionId].messages.length > 0) {
      const sessionMessages = conversationHistory[sessionId].messages;
    ...
    if (status === 429) {
      return res.status(429).json({
        ...
        details: 'You have exceeded the API quota. Please wait a few minutes or try again later.',
        suggestion: 'The API has rate limits. Please wait and try again in a few minutes.',
      });
    }
    ...
    if (status === 400) {
      return res.status(400).json({
        ...
        details: error.message || 'The request format was invalid.',
      });
    }
    ...
    if (status === 401 || status === 403) {
      return res.status(401).json({
        ...
        details: 'Your Claude API key may be invalid or expired. Please check your ANTHROPIC_API_KEY in the .env file.',
      });
    }
    ...
    if (status === 404) {
      return res.status(404).json({
        ...
        details: 'The Claude model you requested is not available. Check if the model name is correct.',
      });
    }

    if (status === 500 || status === 503) {
      return res.status(503).json({
        error: 'Claude API is temporarily unavailable',
        details: 'Anthropic\'s API is experiencing issues. Please try again in a few moments.',
      });
    }
    ...
    if (errorMessage.includes('quota')) {
      userFriendlyMessage = 'You have exceeded your API quota. Please wait a few minutes and try again.';
    } else if (errorMessage.includes('429')) {
      userFriendlyMessage = 'Too many requests. Please slow down and try again in a moment.';
    } else if (errorMessage.includes('404')) {
      userFriendlyMessage = 'The AI model is not available. Please check your API configuration.';
